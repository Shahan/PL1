...
{
    char *PTR;
    /*ðàáî÷.ïåðåìåííàÿ-óêàçàò.*/
//#ifdef COMMENT
    if (flag == 2 || flag == 4) {
        //	  printf("dddd");
        int newCHADR = CHADR;
        if (CHADR % flag) {
            newCHADR = (CHADR /flag + 1) * flag;
        }
        CHADR = newCHADR;
        /*
        if (newCHADR != CHADR) {
            TXT.STR_TXT.DLNOP [1] = newCHADR;
            PTR = (char *)&CHADR;                           
...
int SDC()                                         /*ïîäïð.îáð.ïñ.îïåð.DC    */
{
         ...
         )                                             /* òî                     */
    {
        ...
        RAB = (char *) &RR.OP_RR;                        /*ïðèâåäåíèå ê ñîãëàøåíèÿì*/
...
int SDS()                                         /*ïîäïð.îáð.ïñ.îïåð.DS    */
{
    ...
    if
        (                                             /* åñëè îïåðàíä íà÷èíàåòñÿ*/
         TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[1]=='F' /* ñ êîìáèíàöèè F' èëè 0F */
         )                                             /* òî:                    */
    { 
        RX.OP_RX.B2D2 = 0;                           /*çàíóëèì RX.OP_RX.B2D2   */
        STXT (4,4);                                    /*ôîðìèðîâàíèå TXT-êàðòû  */
        
    } else if
        (                                             /* åñëè îïåðàíä íà÷èíàåòñÿ*/
         TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[1]=='H' /* ñ êîìáèíàöèè F' èëè 0F */
         )                                             /* òî:                    */
    {
        RX.OP_RX.B2D2 = 0;                           /*çàíóëèì RX.OP_RX.B2D2   */
        STXT (2,2);                                    /*ôîðìèðîâàíèå TXT-êàðòû  */
